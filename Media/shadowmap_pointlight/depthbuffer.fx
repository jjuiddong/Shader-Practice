...
float4 PS(VS_OUTPUT In) : SV_TARGET
{
	...
	float depth1 = DepthTexture.Sample(samPoint, float3(In.UV.xy, 0.f)).x;
	float depth2 = DepthTexture.Sample(samPoint, float3(In.UV.xy, 1.f)).x;
	float depth3 = DepthTexture.Sample(samPoint, float3(In.UV.xy, 2.f)).x;
	float depth4 = DepthTexture.Sample(samPoint, float3(In.UV.xy, 3.f)).x;
	float depth5 = DepthTexture.Sample(samPoint, float3(In.UV.xy, 4.f)).x;
	float depth6 = DepthTexture.Sample(samPoint, float3(In.UV.xy, 5.f)).x;
