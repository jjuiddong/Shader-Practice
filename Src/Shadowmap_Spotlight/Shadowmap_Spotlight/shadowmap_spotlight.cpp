...
void cViewer::OnRender(const float deltaSeconds)
{
	...
	if (m_depthBuff.Begin(m_renderer))
	{
		...
		auto ret = m_spotLight[0].GetCameraMatrix(2.0f * m_outerAngle, 1.0f, 0.1f, 1000.f);// m_spotLightRange);
...
void cViewer::RenderSpotLight(const int lightIdx)
{
	...
	auto ret = m_spotLight[lightIdx].GetShadowMatrix(2.f * m_outerAngle, 1.f, 0.1f, 1000.f);// m_spotLightRange);
	...
	XMMATRIX lightWorld = lightTfm.GetMatrixXM();
	XMMATRIX lightView = GetMainCamera().GetViewMatrix().GetMatrixXM();
	XMMATRIX lightProj = GetMainCamera().GetProjectionMatrix().GetMatrixXM();
	m_cbSpotLight.m_v->LightProjection = XMMatrixTranspose(lightWorld * lightView * lightProj);
